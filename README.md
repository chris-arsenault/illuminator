...
into the selected image model's prompt format, then post-process the raw
renders into game-ready assets with embedded provenance metadata and automatic
atlas export for sprites and icons.
...
Generation is no-overwrite by default: existing raw PNG/JSON outputs are
skipped, and only missing `pack.toml` assets are generated. `--and-process`
post-processes only the assets generated by that command and skips existing
processed files. Use `--reprocess` to overwrite the selected raw and processed
outputs.
